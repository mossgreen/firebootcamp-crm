        ...
        <div class="well">
            <form (ngSubmit)="saveCompany()" #companyForm="ngForm">
                <div class="form-group>
                    ...
                    <input type="text" class="form-control" name="name" requried [(ngModel)]="company.name" #name="ngModel">
                    <div [hidden]="name.valid || name.untouched" class="alert alert-danger">Name is requried</div>
                <div>

                <div class="form-group">
                    <label for="email">Email</label>
                    <input type="text" class="form-control" name="email" required [(ngModel)]="company.email" #email="ngModel">
                    <div [hidden]="email.valie || email.untouched" class="alert alert-danger">
                        Email is requried
                    </div>
                </div>
                <button type="submit" class="btn btn-default" [disabled]="!companyForm.form.valid">
                    submit
                <button>
            </form>
        </div>
